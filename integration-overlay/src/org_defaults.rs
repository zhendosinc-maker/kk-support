...
/// Persist first-run defaults on Windows. Test builds keep the decision
/// function independent of hbb_common for standalone rustc execution.
...
pub fn apply_once() {
    ...
    for (name, value) in decide_defaults(&options) {
        Config::set_option(name.to_owned(), value);
    }
    Config::set_option(ORG_DEFAULTS_MARKER.to_owned(), "1".to_owned());
}
